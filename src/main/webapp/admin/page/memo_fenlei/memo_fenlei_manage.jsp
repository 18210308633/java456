...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/java456/houtai/memo/fenlei/add' //iframe的url
	});
}
...
function edit(id){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '/java456/houtai/memo/fenlei/edit?id='+id //iframe的url
	});
}
...
function del(ids){
	...
	$.post('/java456/admin/memo/fenlei/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
...
function table_edit_update(id,field,value){
	$.post('/java456/admin/memo/fenlei/update?'+field+'='+value,{id:id},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.closeAll();
...
function update_isUse(id,isUse){
	$.post('/java456/admin/memo/fenlei/update',{id:id,isUse:isUse},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.msg('修改失败');
...
<script>
		layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel',
			...
			, element = layui.element; //元素操作
			  table.render({
			    elem: '#table'
			    ,url: '/java456/admin/memo/fenlei/list',
			    height: 'full-60',
			   cols: [[

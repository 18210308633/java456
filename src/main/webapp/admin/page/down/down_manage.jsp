...
//打开添加窗口
function add(){
	window.open("/java456/houtai/down/add");
}

//打开编辑窗口
function edit(id){
	window.open('/java456/houtai/down/edit?id='+id);
}
...
function del(ids){
	...
	$.post('/java456/admin/down/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
...
function table_edit_update(id,field,value){
	$.post('/java456/admin/down/update?'+field+'='+value,{id:id},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.closeAll();
...
function update_isUse(id,isUse){
	$.post('/java456/admin/down/update',{id:id,isUse:isUse},function(result){
		if(result.success){
			layer.msg('修改成功');
		}else{
			layer.msg('修改失败');
